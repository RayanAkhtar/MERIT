\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % II. Configuration (Moved lower to align with audit)
        \node [arch_logic, right=2.2cm of db1] (fusion) {Weighted Metric\\Fusion Engine};
        
        % III. Execution
        \node [arch_logic, right=2.2cm of fusion, yshift=1.6cm] (run) {Semantic\\Scoring Pipeline};
        \node [arch_logic, below=0.5cm of run] (audit) {Explainability \&\\Audit Trail};
        ...
        \node [domain_label, fill=white, inner sep=2pt] at (area1.north) {I. EXTRACTION};
        \node [domain_label, fill=white, inner sep=2pt] at (area2.north) {II. CONFIGURATION};
        \node [domain_label, fill=white, inner sep=2pt] at (area3.north) {III. EXECUTION};

    \end{tikzpicture}
    \caption{High-Level System Design: Multi-source Extraction, Job Configuration, 
    and Job Execution.}
