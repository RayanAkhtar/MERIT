...
MERIT uses the Python libraries \texttt{pdfplumber} and \texttt{pdfminer} \cite{pdfminer} to extract text from CV and JD PDFs.
These tools often leave parsing noise and unwanted characters, so the raw text is sanitised by removing null bytes
(\texttt{\textbackslash u0000}) and redundant whitespace before downstream extraction.
...
For MERIT's ingestion stage, we follow a similar approach to that of other existing ATS systems.
...
Devicon is limited to programming languages and also frameworks, but fails to include observability tooling,
...
with strict word boundaries (\texttt{\textbackslash b}) so that, for example, ``C++'' does not match ``C'' and ``R'' does not match within ``Ruby''.
...
delimiters to split text blocks and bullet points, allowing us to extract the roles, companies and institutions. We also use
regular expressions to extract date ranges as well.
...
For each configured job, we compute the $W_s$ scores for each skill; this may also be commonly referred to as an extensible metric.
