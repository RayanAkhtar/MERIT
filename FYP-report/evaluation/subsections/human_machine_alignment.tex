...
engine rankings against recruiter consensus, using the same baselines as Study~01. Consequently, Studies~07--10 serve as the final human-centric evaluation for \textbf{RQ2 (Multi-Source Fusion)}, determining whether the ranking output of the engine actually aligns with expert human judgement.
...
Recruiter consensus, however, is formed from CV PDFs only.
To reiterate, rank~1 is the best fit for a candidate.
...
First, we computed each candidate's mean rank across recruiters.
...
\end{equation}
where $h_i$ is the human expert consensus rank for candidate $i$, $e_i$ is the automated engine rank for candidate $i$, and $n$ is the total number of candidates in the cohort ($n=10$).

We use \texttt{scipy.stats.spearmanr} to compute the $\rho$ and two-sided $p$-values. Note that due to the small sample size
($n=10$), a single rank swap can move $\rho$ sharply, for a similar reason, the $p$-values are exploratory. To reiterate, the engines evaluated here are the same four baselines established in Study~01 (Section~\ref{subsec:study-01-comparative-ablation}).
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c c}
        ...
        MERIT (CV Only) & 0.5152 ($p{=}0.128$) & \textbf{$-$0.5030 ($p{=}0.138$)} & 0.0909 ($p{=}0.803$) & 0.5152 ($p{=}0.128$) \\
        MERIT (Full)    & \textbf{0.7333} ($p{=}0.016$) & \textbf{$-$0.5030 ($p{=}0.138$)} & 0.7212 ($p{=}0.019$) & 0.0909 ($p{=}0.803$) \\
        \bottomrule
    \end{tabular}
\end{table}
...
\textbf{Outcome:} Does not meet $\rho \geq 0.7$ for any engine (MERIT Full $\rho = -0.503$), seniority-bias stress test failed.
