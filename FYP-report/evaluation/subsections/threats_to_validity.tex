...
to which the metrics and instruments we use measure the research questions we claim to address.
...
Another threat is that our Human-Machine Alignment studies (Study~07--10) use pre-parsed JSON data that assumes
...
We also retain match scores, which may also be anchored by the participants of the study. We cannot pass off the results of this
...
We do not implement a rule-based ATS baseline since it is knockout-oriented rather than ranking-oriented. 
...
The Modern AI baseline uses SBERT to compute cosine similarities between CV embeddings and JD embeddings, however, real-world
...
scores are likely to vary from run to run, however, the general pattern will remain stable across configurations under identical hardware and software.

\textbf{Algorithmic hyperparameters and weighting.}
MERIT is a customisable engine, recruiters are allowed to adjust the weights of the metrics to suit their needs.
For the purposes of this evaluation, we set the weights to reflect our interpretation of an evidence-first
engine, we weighted metrics such as languages and projects higher than education and experience.
These weights are not empirically validated. Since candidate rankings are sensitive to these hyperparameters,
both the score gaps and rank shifts are only valid for the specific weights used in this evaluation. A different
set of weights would likely alter the magnitude of the rank shifts observed, for example, Carl Corp would 
rank higher if we weighted experience above languages and projects.
...
Also, Study~14 always runs the black-box stage before the glass-box stage, the reversals and confidence
...
Here, we address whether or not the conclusions that we draw from our data align with our choice of methods and metrics.
...
We have outlined many threats to validity throughout this chapter, however, 
the evaluation still provides significant directional evidence under controlled data.
...
When supplementary evidence agrees with the CV, we claim that fusion can improve alignment with expert ordering.
...
MERIT performs poorly under batch-wide contradictory evidence (Study~10).
We cannot claim that MERIT works well when presented with nuances in the hiring process such as the concept of overqualification 
(Section~\ref{subsec:study-08-spearman-seniority}).
