...
\textbf{Skill dictionary limitations.} Skill extraction is not hindered by the layout of a CV, but rather the dictionary that it is 
...
\textbf{Structural layout failures.} The detection of candidate name drops sharply under multi-column layouts.
...
\textbf{Regex robustness.} Email extraction is fully robust, achieving a 100\% match rate across both layout types,
confirming that our regex-based extraction process is not hindered by the layout of the candidate CV.

\textbf{Inline vs.\ multi-line parsing.} We observe counterintuitive results for experience extraction, noting that experience recall rises from
34.3\% in single-column layouts to 44.7\% in multi-column layouts. Upon inspection, this is likely because single-column layouts often present experience as inline, with
...
separate lines. This makes it easier for the parser to isolate details, which could suggest why multi-column
layouts have better recall for experience compared to single-column layouts.

\textbf{The systemic implications of these findings are significant.} CVs are unstructured documents, and as 
such, there is a need for structured, schema-based data sources to be able
to account for parsing errors that may arise from CV ingestion (such as a LinkedIn profile). Commercial ATS likely suffer from similar structural parsing failures, but their black-box nature obscures these errors from recruiters.
...
\textbf{Lexical contamination.} Unlike CVs, which serve as concise professional summaries, Job Descriptions (JDs) are inherently
...
\textbf{Image-based employer branding.} The most notable observation within this evaluation is the poor performance of company name extraction, with a match rate
of 6.67\%, only successfully extracting one of the fifteen JD company names. The reason for this is structural, rather than 
semantic. Employers often represent company names in text that is not easily extractable by a parser, for example, they
...
When generating the market corpus, we employ Tonic.ai \cite{tonicai} to synthesise a dataset of 100 JDs, providing a relatively realistic
...
Frontend, senior ML and Lead DevOps role. This is done to prevent their influence on the IDF scores. We have one global corpus,
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c c c}
        \toprule
        \textbf{Hold-out role} & \textbf{Skills} & \textbf{Exact match} & \textbf{Close match ($\pm 1$)} & \textbf{MAE} & \textbf{Spearman $\rho$} \\
        \midrule
        Junior Frontend (React) & 9  & 44.4\% & 77.8\% & 0.78 & 0.52 \\
        Senior ML Engineer       & 8  & 25.0\% & 50.0\% & 1.88 & $-0.45$ \\
        Lead DevOps / Platform   & 10 & 20.0\% & 60.0\% & 1.20 & 0.40 \\
        \midrule
        \textbf{Aggregate} ($n{=}27$) & 27 & 29.6\% & 63.0\% & 1.26 & 0.25 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\textbf{Small sample size.} This is a pilot study ($N = 4$ evaluators, $n = 27$ pairs), and the results are not generalisable to the wider market.
We are not able to conduct a larger panel study due to the time and resource constraints of this project.

\textbf{Cross-role IDF interference.} Another limitation is due to the way we create the corpus, or rather, the fact that we only use one corpus for all TF-IDF calculations.
...
proportion of job descriptions and, as a result, we would have a low IDF score, regardless of the role, making it seem like Python is 
always a common skill. This down-ranks Python on the metric predictions, regardless of the role.

\textbf{Scale-use asymmetry.} In this study, we also did not ask our participants to spread their ratings evenly between the 1 and 5 scale. Compare this to TF-IDF
...
\textbf{Multi-Source Fusion (Pillar 2).} The recall of 38.6\% means that CV-only screening will often miss the majority of a candidate's technical competencies
...
\textbf{Glass-Box Explainability (Pillar 3).} The issues presented above would often fail to be surfaced to recruiters in typical black-box scoring systems.
