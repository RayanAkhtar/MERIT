...
This section evaluates the third pillar of MERIT, addressing RQ3. We first verify the mathematical correctness of the Shapley values,
applying the coalition Shapley formalism from explainable ML~\cite{lundberg2017unified} at the data-source level.
From here, we conduct a user study to understand the impact of the explainability layer on recruiter decision-making. In that study, we also
...
contributions for MERIT are heterogeneous by design, and therefore cannot produce identical marginal contributions in practice.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c c c}
        \toprule
        \textbf{Candidate} & $\phi_{\mathrm{CV}}$ & $\phi_{\mathrm{GH}}$ & $\phi_{\mathrm{LI}}$ & $\sum \phi_i$ & $v(N)$ \\ \midrule
        ...
        Vince Vault   & +0.2652 & $-$0.0708 & +0.0117 & 0.2060 & 0.2060 \\ \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Per-metric efficiency:} The efficiency axiom also holds at the granular skill level. Per-metric breakdowns are
    provided in Appendix~\ref{appendix:shapley-charts} for two candidates (Jordan Smith and Felix Vance).
\end{itemize}
...
\textbf{Negative attributions.} One key finding from this study is visible in the GitHub Shapley value for Vince Vault, where we show the presence of negative attributions.
...
it was due to identity squatting, leading to a significant penalty being applied to the candidate's score.
Since Vince provided false evidence only for the GitHub source, negative attributions were focused there, rather than on LinkedIn or CV.
This penalty is then presented to the recruiter to show that a candidate might be acting with malicious intent.
It is now the recruiter's responsibility to investigate the flag further to see if the fuzzy match failed due to identity squatting, 
or due to a false positive match.

\textbf{Demonstrated source dominance.} Another finding can be presented through our hidden gem in Section~\ref{subsec:study-01-comparative-ablation},
Jordan Smith. As shown in that study, the pivot from MERIT (CV Only) to MERIT (Full) boosts his score from rank 10 to rank 2. Due
to the introduction of Shapley values, we can confidently claim that this boost in rank is due to the inclusion of the GitHub data source,
which contributed $\phi_{GitHub} = 0.5288$ to his score. This is presented to the recruiter on the frontend to show that the majority
...
see if they are a good fit for the role, since they have demonstrated well that they align with the job, but their claims might not be as strong.

\textbf{Signal starvation.} It is important to note that the dataset for this study reuses that of Study~01, therefore, we do not have significant Shapley values for
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c}
        \toprule
        \textbf{Source} & \textbf{Sum of positive $\phi_i$} & \textbf{Share of pool (\%)} \\ \midrule
        CV (claimed)       & 2.429 & 59.6 \\
        GitHub (demonstrated) & 1.564 & 38.4 \\
        LinkedIn (claimed)    & 0.085 & 2.1 \\ \bottomrule
    \end{tabular}
\end{table}

\textbf{Aggregate source dominance.} We can observe that in this cohort, CV evidence dominates the overall score, contributing to 59.6\% of the overall score of 
...
factor in the candidate's score. Note the margin for error since we only consider positive $\phi_i$ for the denominator (we exclude the 
negative GitHub attribution from Vince Vault). This exclusion is mathematically necessary to prevent the net sum from shrinking, 
which would cause the relative percentage shares to artificially inflate or exceed 100\%, ensuring the breakdown strictly measures 
the composition of \textit{positive} score generation.

\subsection{Study 14: HCI Trial}
\label{subsec:hci-trial-protocol}

Study 11 mathematically confirms that the explainable AI (XAI) engine is mathematically consistent, this study evaluates the human-facing 
...
We also measure how usable this engine is since, if the system is too difficult to use there is a risk that recruiters would 
avoid using it, falling back into automation bias to avoid significant cognitive load. Therefore, we have the following hypotheses:

\textbf{Hypothesis 1 (Decision Calibration):} Access to the Glass-Box XAI audit trail will increase the rate at which recruiters reverse incorrect screening decisions for adversarial candidates (such as keyword-stuffers and identity squatters) and ``hidden gems'' (like high-potential candidates with low tenure).

\textbf{Hypothesis 2 (Confidence Calibration):} Recruiters will exhibit a higher alignment between their screening confidence and the empirical evidence, resulting in increased confidence for verified candidates and decreased confidence for unverified profiles.

\textbf{Hypothesis 3 (Usability and Trust):} The interactive explainability layer will achieve a high System Usability Scale (SUS) score and receive high qualitative ratings for transparency and defensibility compared to a traditional black-box score.
...
\textbf{Stage 1: Control Condition (Black-Box Score).} Recruiters are given a candidate CV with a single aggregate match alongside per-metric scores, but with no explanations.
...
\textbf{Stage 2: Treatment Condition (Glass-Box Scorecard).} Recruiters are now shown the same candidate, but with access to the glass-box scorecards, including the Shapley layer, audit trail,
...
These participants are familiar with the technical recruitment process or have undergone multiple technical projects and internships. 
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c c c c}
        \toprule
        \textbf{Candidate}   & \textbf{Stage 1 (ctrl)} & \textbf{Stage 2 (trt)} & \textbf{DRR} & \textbf{$C_1$ (1--5)} & \textbf{$C_2$ (1--5)} & \textbf{$\Delta C$} \\ \midrule
        ...
        Aaron Mohammed       & $3S$ / $2R$            & $0S$ / $5R$            & 100.0\%      & 3.20                    & 4.80                    & +1.60                 \\ \bottomrule
    \end{tabular}
\end{table}
...
\textbf{The ``resume black hole'' (Yusuf Nilsson).} During the first stage, we can see the issue of the ``resume black hole'': all 5 evaluators rejected Yusuf Nilsson
...
\textbf{Keyword Stuffing (Robert Todd).} Another key finding is that the explainability layer successfully aided in unmasking adversarial manipulation. More specifically,
we turn to the candidate Robert Todd, who managed to fool 4 out of 5 evaluators in stage 1 due to his keyword-dense
...
\textbf{Identity Squatting (Aaron Mohammed).} The most significant instance of this is with the candidate Aaron Mohammed, who managed to fool 3 out of 5 evaluators
in stage 1 due to his misleading CV. However, upon transitioning to stage 2, rather than seeing supported evidence, recruiters were
presented with a significant red flag. Aaron provided a GitHub profile that was not his own in order to pass off
...
3 evaluators reversed their decision, rejecting Aaron. It is crucial to note that recruiters did not reject
Aaron based on the fact that he was not a good developer (in fact, according to his CV he was an expert in Java and Go).
...
can be used to identify and reject candidates who are not a good fit for the company culture, even if they are technically skilled and achieve
high match scores.


\subsubsection{Confidence Calibration (Hypothesis 2)}
\textbf{Individual Confidence shifts.} Once evaluators were presented with the Glass-Box MERIT interface, we observed a positive confidence shift ($\Delta C$)
...
\textbf{Aggregate Confidence shifts.} The average confidence across all 20 profile-level observations (5 participants $\times$ 4 candidates) increased from $3.75$ 
...
\textbf{System Usability Scale (SUS).} To evaluate the usability of MERIT's glass-box architecture, we followed the standard System Usability Scale (SUS) survey \cite{brooke1996sus}.
...
that still shortlisted Robert Todd after the keyword-stuffing alert. Despite what is considered a ``red flag'' by $P_3$, they still plan to 
