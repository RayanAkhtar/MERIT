...
aid candidate scoring? Gugnani and Misra~\cite{gugnani2020} and Lo et al.~\cite{lo2025aihiring} criticise
...
\textbf{RQ2 (Multi-source):} Does evidence-aware multi-source fusion improve candidate ranking over
...
\textbf{RQ3 (Transparency):} Can glass-box explainability and transparency aid recruiter decision calibration relative
to that of a black-box system? This RQ aims to address the transparency deficits that are documented in 
modern algorithmic hiring reviews~\cite{bogen2018,raghavan2020}. We also apply the coalition Shapley
formalism from explainable ML~\cite{lundberg2017unified} at the data-source level 
(Section~\ref{sec:explainable-ai-shapley}).
...
\begin{table}[H]
    \centering
    \caption[Evaluation study catalogue]{Evaluation study catalog, prefixing each study by order of
    implementation. Studies~02, ~03 and~06 are engineering or RQ supplements, 
    they do not contribute to an RQ's success criteria.}
...
\begin{table}[H]
    ...
    Temporal skill decay~\cite{arthur1998} is analysed in Section~\ref{subsec:study-08-skill-decay} as an 
    RQ2 supporting analysis alongside the fusion studies.
