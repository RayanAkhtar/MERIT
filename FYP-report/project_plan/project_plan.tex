...
The main objective is to design and implement a multi-source screening system for recruiters, where this entails combining multiple data sources to gain more insight into a candidate and evaluate their technical proficiencies. 
Extensibility will also be a core focus so that additional data sources can be added with minimal development.
For the initial implementation, there will be attempts to support CVs as well as LinkedIn and GitHub profiles.
...
MERIT will be able to infer evaluation metrics from the job description, allowing users to add, remove, or edit these metrics before running the screening process.
...
\begin{itemize}
  ...
  \item \textbf{Automated Flagging/Tagging -} Highlight potential issues such as large employment gaps or irregular career patterns. Also highlighting users that have desired traits common in successful candidates (e.g. fast at picking up languages, high career trajectory etc)
  ...
  \item \textbf{Language similarity mapping -} Identify, and tag transferable language skills (e.g. C\(\leftrightarrow\)C++/Rust).
  ...
  \item \textbf{Domain-specific skill classification -} Distinguish between subdomains of the same technology (e.g. Python for ML vs Backend)
...
\begin{tabularx}{\textwidth}{|l|X|X|}
  ...
  \hline
  Mid February - Mid March & multi-source Ingestion complete + basic scoring engine & Finish LinkedIn and GitHub implementation and implement initial scoring logic with metric customisation \\
  \hline
...
For the multi-source ingestion and basic scoring, if time does not permit, there will be a fallback on a fixed scoring template rather than allowing for custom metrics.
