...
A key challenge in developing MERIT will be to address the uncertainty in conflicting signals that arise from handling multi-source candidate data.
...
This distribution is well-suited for the task since it can be updated incrementally as new evidence arrives.
...
This also prevents significant penalties for candidates who may not have all the evidence sources available to them.
...
The fused match score and confidence (variance) are:
...
following standard Beta--Bernoulli updating \cite{gelman2013bayesian}.
