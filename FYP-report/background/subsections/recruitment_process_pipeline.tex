...
When applying for technical positions, candidates will typically progress through one or more evaluation stages.
It is common for individual firms to omit optional stages or follow a different ordering \cite{cappelli2019}.
...
Online technical assessments (OAs) are core to the hiring process for many organisations. 
...
Historical data highlights that the majority of firms (80\% of small and 66\% of large firms) place high value on portfolio evidence,
...
An assessment centre (AC) is a structured multi-exercise evaluation process, often used in graduate programs and enterprise-level hiring. 
...
Being the first stage in the recruitment pipeline, screening can be seen as the most influential filter in the process.
It often rejects the majority of candidates so that later stages can focus on stronger fits, yet organisations 
still rely on a single unstructured source, the CV, while richer evidence (such as GitHub and LinkedIn) are 
deferred or applied inconsistently. MERIT takes this into account in its design, the pivot in demand for demonstrable evidence 
brings about the need for a system that can understand a candidate's proficiency, rather than relying on self-reported claims.
