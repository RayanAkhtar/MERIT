...
ATS platforms can be categorised in various ways, but in our report we will categorise it based off the underlying
...
Rule-Based ATS refer to systems that focus on explicit, pre-defined rules or criteria in order to 
...
There is also the improved ability in matching candidates who used varied phrasing, and also an increase in
the transparency of the system compared to the AI-driven systems, since much of the logic is still rule-based.
...
AI-Assisted ATS comprise of machine learning models that train on previous hiring data to
predict candidate-job fit. The system does not follow that of its predecessors, instead
inferring relationships between the skills, roles and qualifications that led to
successful hires in the past.

Due to this, the systems employ vector-embeddings and semantic search to evaluate whether
a candidate is similar to other successful candidates for that role. They also take advantage
...
matchmaking accuracy, as well as detection of transferrable skills. They are also able to handle 
...
Furthermore, the “black box” nature of these systems lead to low transparency, as there 
is often little explanation for ranking and rejecting decisions, undermining both the 
...
MERIT addresses this through the introduction of a multi-source screening system that integrates both
demonstrated and claimed evidence from candidate data to provide a more thorough assessment of a candidate.
We also introduce both transparency and explainability into the system to account for the limitations of the 
previous systems. This provides recruiters with the tools necessary to ground their decisions in observable
performance rather than trust in self-reported claims. It also allows them to interrogate the system
and understand why a candidate was given a certain rank by our system, and if neccesary, challenge the system's decision.