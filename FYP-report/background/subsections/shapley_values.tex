\subsection*{Shapley Values for Data Source Attribution}
...
Part of that requires the system to be able to capture the contribution of each data source to the final match score.
...
The left side of the summation is the combinatorial weighting factor, which weights each coalition of the remaining sources according to its size. The right side of the summation is the marginal contribution of the source $i$ to the coalition $S$.
...
By building on the SHAP (Shapley Additive Explanations) framework \cite{lundberg2017unified}, MERIT applies 
the same Shapley formalism at the data-source level to attribute each coalition's marginal contribution to the 
final score. Note that Lundberg and Lee developed this approach to explain individual model predictions, whereas 
MERIT applies it across data-source coalitions.
