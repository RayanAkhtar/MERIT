...
to combat reliance on a singular self-reported data source that is often subject to parsing errors and embellishment, as well as 
the lack of flexibility and transparency of the current state of candidate screening systems. The key contributions
to meet our objectives were extensible metrics, data fusion and glass-box explainability to combat these core limitations. 
We will now cover the strengths and weaknesses of our approach to each of the three pillars.
...
Extensibility is real, but it is often configured on misread documents, confirming ResumeBench's documented parsing 
...
\textbf{Worked:} Study~01 helps to recover hidden talent (Jordan Smith, rank~\#10 $\rightarrow$~\#2) and penalise fraud when GitHub ownership is checked
(Vince Vault, rank~\#4 $\rightarrow$~\#9). Study~07 gives the best observed expert Spearman alignment from MERIT in one cohort ($\rho = 0.733$, $n=10$,
exploratory), partially validating Montandon et al.'s~\cite{montandon2021_roles} supplementary-GitHub hypothesis.
...
Fusion is strongly dependent on the presence of strong, reliable evidence that is accessible across the entire candidate pool.
