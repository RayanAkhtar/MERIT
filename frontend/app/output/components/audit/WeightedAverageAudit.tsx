          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {(m.weighted_average_breakdown || []).map((wa: any, waIdx: number) => (
                <div key={waIdx} className="flex justify-between items-center text-xs font-mono p-2 bg-indigo-500/5 rounded border border-indigo-500/10">
                  <span className="text-zinc-900 dark:text-zinc-300">{wa.name} Score:</span>
                  <span className="text-zinc-900 dark:text-white font-bold">{Math.round((wa.score || 0) * 100)}%</span>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 gap-1">
              {(m.weighted_average_breakdown || []).map((wa: any, waIdx: number) => (
                <div key={waIdx} className="flex justify-between items-center text-xs font-mono px-2 py-1 opacity-80">
                  <span className="text-zinc-900 dark:text-zinc-300">{wa.name}:</span>
                  <span className="text-indigo-950 dark:text-indigo-200 italic">{(wa.score || 0).toFixed(2)} * {(wa.weight || 1).toFixed(1)} = {((wa.score || 0) * (wa.weight || 1)).toFixed(2)}</span>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-center space-y-2">
               <div className="text-xs font-mono text-indigo-950 dark:text-indigo-300 uppercase tracking-widest font-bold">Sum(Weighted Scores) / Sum(Weights)</div>
                <div className="text-base font-mono text-zinc-900 dark:text-white font-black">
