      ...
      <div className="p-4 rounded-xl bg-gradient-to-br from-indigo-500/10 to-transparent border border-indigo-500/20 shadow-inner space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col p-2 bg-indigo-500/5 rounded border border-indigo-500/10">
            <span className="text-[9px] uppercase text-zinc-900 dark:text-zinc-500 font-bold mb-1">Decay Constant (λ)</span>
            <span className="text-xs font-mono text-indigo-950 dark:text-indigo-300">{item.temporal_params?.lambda}</span>
          </div>
          <div className="flex flex-col p-2 bg-indigo-500/5 rounded border border-indigo-500/10">
            <span className="text-[9px] uppercase text-zinc-900 dark:text-zinc-500 font-bold mb-1">Temporal Delta (Δt)</span>
            <span className="text-xs font-mono text-indigo-950 dark:text-indigo-300">{item.temporal_params?.delta_t} years</span>
          </div>
        </div>
        ...
        {item.temporal_params?.history && item.temporal_params.history.length > 0 && (
          <div className="mt-2 p-3 bg-indigo-500/5 dark:bg-black/20 rounded border border-indigo-500/10 dark:border-white/5 space-y-2">
            <div className="text-[9px] uppercase text-black dark:text-zinc-500 font-bold tracking-widest border-b border-indigo-500/10 dark:border-white/5 pb-1.5 mb-1.5">Δt Derivation (Volume Weighted)</div>
            <div className="space-y-1">
              {item.temporal_params.history.map((h: any, i: number) => (
                <div key={i} className="flex justify-between items-center text-[10px] font-mono text-zinc-900 dark:text-zinc-400">
                  <span>{h.year} Volume:</span>
                  <span className="text-indigo-950 dark:text-indigo-300">{h.volume} LOC</span>
                </div>
              ))}
            </div>
            <div className="border-t border-indigo-500/10 dark:border-white/5 pt-2 mt-2 pb-1 mb-1 text-[10px] font-mono text-black dark:text-zinc-500 flex flex-col gap-1 italic">
            ...
            <div className="border-t border-indigo-500/10 dark:border-white/5 pt-1.5 mt-1 text-[10px] font-mono text-black dark:text-zinc-400 flex flex-col gap-1.5">
              <div className="flex justify-between">
                <span>Effective Year (Weighted):</span>
                <span className="text-indigo-950 dark:text-indigo-300">{item.temporal_params.effective_year}</span>
              </div>
              <div className="flex justify-between">
                <span>Current Year:</span>
                <span className="text-indigo-950 dark:text-indigo-300">{item.temporal_params.current_year}</span>
              </div>
              <div className="flex justify-between font-bold mt-1">
                <span>Δt ({item.temporal_params.current_year} - {item.temporal_params.effective_year}):</span>
                <span className="text-indigo-950 dark:text-indigo-300">{item.temporal_params.delta_t} years</span>
              </div>
            </div>
          </div>
        )}
        <div className="pt-2 border-t border-indigo-500/10 flex justify-between items-center bg-indigo-500/5 dark:bg-white/5 px-2 py-1 rounded">
          <span className="text-[10px] text-black dark:text-zinc-500 font-mono uppercase font-bold">Substitution:</span>
          <span className="text-sm font-mono text-indigo-950 dark:text-indigo-300">
        ...
        <p className="text-[10px] text-zinc-900 dark:text-zinc-400 leading-relaxed text-center px-2">
