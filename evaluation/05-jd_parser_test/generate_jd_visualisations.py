...
def generate_accuracy_plots():
    ...
    if not os.path.exists(summary_path):
        print("Error: JD Accuracy summary not found. Run the test first.")
        return
    ...
    colors = [ICL_LIGHT_BLUE] * len(averages)
    bars = ax.bar(averages.index, averages.values, color=colors, alpha=0.8, edgecolor='black')
    
    ax.set_title('JD Parser Performance Metrics (Study 05)', fontsize=14, fontweight='bold', pad=20)
    ...
    plt.tight_layout()
    output_path = os.path.join(current_dir, "output/jd_parser_accuracy.png")
    plt.savefig(output_path, bbox_inches='tight')
    plt.close(fig)
    print(f"[SUCCESS] JD Parser Accuracy plot saved to {output_path}")
